...
public:
	...
	camera()
	{
		up = down = left = right= w = a = s = d = q = e = z = c = 0;
		pos = rot = glm::vec3(0, 0, 0);
	}
	glm::mat4 process(double ftime)
	{
		float speed, xAxis, yAxis;
		speed = xAxis = yAxis = 0;
		...
		float yangle=0;
		if (a == 1)
			yangle = -0.2* fwdspeed *ftime;
		else if(d==1)
			yangle = 0.2 * fwdspeed *ftime;
		rot.y += yangle;
		float zangle = 0;
		if (q == 1)
			zangle = -0.2 * fwdspeed* ftime;
		else if (e == 1)
			zangle = 0.2 * fwdspeed* ftime;
		rot.z += zangle;
		float xangle = 0;
		if (z == 1)
			xangle = -0.2* fwdspeed * ftime;
		else if (c == 1)
			xangle = 0.2 * fwdspeed* ftime;
		rot.x += xangle;
		...
		glm::vec4 dir = glm::vec4(xAxis, yAxis, speed,1);
		R = Rx * Rz * R;
		dir = dir*R;
...
/*
	...
	//	{ { 66.9388,-5.50663,345.286 },
	//	{ 0.686623,0,0.727014 },
	//	{ 0.189292,0.965509,-0.178775 } },

//		{ { 70.2665,-5.26928,350.941 },
//		{ 0.756459,0.00303282,0.654034 },
//		{ 0.198494,0.95176,-0.233992 } },
...
//end//
		...
		*/
		...
		{ -0.422457,0.897358,0.12759 } },
			/*
		{ { 33.3556,-9.87038,336.767 },
		{ 0.857529,0.429715,-0.282824 },
		{ -0.402517,0.902825,0.151288 } }, */

		{ { 42.2401,-7.32628,336.778 },
		{ 0.924741,0.292151,0.24393 },
		{ -0.286687,0.95624,-0.0584376 } },
/*
		{ { 63.4317,-3.36581,343.16 },
		{ 0.801384,0,0.59815 },
		{ 0.337712,0.825369,-0.452457 } },
*/
	...
	********/
	void render()
	{
		...
		if (cam_persp) {
			...
			V = setupCamAlongPath2(frametime);
		}
		M = glm::mat4(1);
		...
		//draw the dot
		TransZ = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -0.1f, -0.5f));
		S = glm::scale(glm::mat4(1.0f), glm::vec3(0.4f));
		float dangle = 3.1415926 / 2.;
		glm::mat4 RotateXDot = glm::rotate(glm::mat4(1.0f), dangle, vec3(1, 0, 0));
		dangle = 3.1415926;
		glm::mat4 RotateYDot = glm::rotate(glm::mat4(1.0f), dangle, vec3(0, 1, 0));

		M = Vi *TransZ * RotateXDot * RotateYDot * S;
		glUniformMatrix4fv(skyprog->getUniform("M"), 1, GL_FALSE, &M[0][0]);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, 0);
		//plane->draw(skyprog, false);
